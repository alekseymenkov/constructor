    ...
    mClientData(0)
{
    ...
    connect(this, SIGNAL(signalDataRequest(quint16,quint16,quint16,quint16)),
            this, SLOT(slotDataResponse(quint16,quint16,quint16,quint16)));
}
...
int Server::start()
{
    ...
    for (int i = 0; i < mMaxClients; i++)
    {
        ...
        mClientData[i].isForWrite.clear();
...
int Server::disconnectClient(quint16 clientNumber)
{
    ...
    mClientData[clientNumber].isForWrite.clear();
...
int Server::slotDataResponse(quint16 clientNumber, quint16 requestID,
                             quint16 startByte, quint16 bytesCount)
{
    // Формирование данных
    QByteArray data = mClientMMF[clientNumber]->read(startByte, bytesCount);
    QByteArray package = createDataResponsePackage(requestID, data);

    // Отправка данных
    if (!mClients[clientNumber].isNull())
        mClients[clientNumber].data()->write(package);
...
int Server::slotReceiveData()
{
    ...
    while (true)
    {
        ...
        case PackageType::Data:
            result = this->parseDataPackage(clientNumber, package);
            break;
        ...
        case PackageType::CoefficientRequest:
            ...
            break;
        case PackageType::SingleDataRequest:
            result = this->parseDataRequestPackage(clientNumber, package);
            break;
...
bool Server::parseRegInfoPackage(quint16 clientNumber, QByteArray data)
{
    quint16 id = 0;
    quint16 startByte = 0;
    ...
    // Опциональная часть - режим передачи (1 байта)
    if (data.size() % 2 != 0)
    {
        memcpy(&isForWrite, data.data(), sizeof(quint8));
        data = data.mid(sizeof(quint8));
    }
    ...
    mClientData[clientNumber].clientData.clear();
    mClientData[clientNumber].isForWrite.append(isForWrite);
    mClientData[clientNumber].count++;

    bool isError = false;

    // Вывод информации о зарегистрированном элементе
    QString infoString = QString("Клиент №%1. Регистрация элемента №%2. Диапазон данных (в байтах): ")
            .arg(clientNumber)
            .arg(id);
...
bool Server::parseDataRequestPackage(quint16 clientNumber, QByteArray data)
{
    quint16 id = 0;
    quint16 startByte = 0;
    quint16 bytesCount = 0;
    ...
    // Получение идентификатора запроса (2 байта)
    memcpy(&id, data.data(), sizeof(quint16));
    data = data.mid(sizeof(quint16));
    ...
    // Вывод информации о зарегистрированном элементе
    QString infoString = QString("Клиент №%1. Запрос данных из отображаемого в память файла. Диапазон данных (в байтах): ")
            .arg(clientNumber);
    ...
    // Определение границ запрашиваемой области данных
    int endByte = startByte + bytesCount - 1;
    // Проверка, что не запрошена нулевая область данных
    if (endByte < startByte)
        isError = true;
    ...
    // Два формата вывода информации о запрашиваемых данных
    if (startByte == endByte)
        infoString += QString("%1").arg(startByte);
    else
        infoString += QString("%1 - %2").arg(startByte).arg(endByte);
    ...
    emit signalDataRequest(clientNumber, id, startByte, bytesCount);
...
bool Server::parseDataPackage(quint16 clientNumber, QByteArray package)
{
    quint16 elementID = 0;
    // Байты 0 - 1: идентификатор элемента
    memcpy(&elementID, package.data(), sizeof(quint16));
    // Байты 2 - n: данные для MMF
    package = package.mid(sizeof(quint16));

    QByteArray dataToMMF;
    for (int j = 0; j < mClientData[clientNumber].parts[elementID]; j++)
    {
        const int bytesCount = mClientData[clientNumber].bytesCount[elementID][j];
        if (package.size() < bytesCount)
            return false;

        dataToMMF = package.mid(0, bytesCount);
        mClientMMF[clientNumber]->write(mClientData[clientNumber].startByte[elementID][j],
                                        dataToMMF);

        package = package.mid(bytesCount);
    }
...
QByteArray Server::createDataPackage(quint16 clientNumber)
{
    QByteArray data;
    ...
    {
        if (mClientData[clientNumber].isForWrite[i])
            continue;
...
// Генерация пакета SingleDataResponse
QByteArray Server::createDataResponsePackage(quint16 requestID, QByteArray data)
{
    QByteArray package;
    quint16 sizeOfPackage = 2 * sizeof(quint16) + sizeof(quint8) + data.size();
    quint8 packageType = PackageType::SingleDataResponse;
