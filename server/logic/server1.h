...
private:
    ...
    bool parseRegInfoFromServerPackage(quint16 clientNumber, QByteArray data);
    // Разбор пакета, содержащего информацию о регистрации клиентского элемента
    // Данные передаются клиента к серверу
    bool parseRegInfoToServerPackage(quint16 clientNumber, QByteArray data);
    ...
    bool parseDataToServerPackage(quint16 clientNumber, QByteArray data);
    ...
    QByteArray createDataFromServerPackage(quint16 clientNumber);
    // Генерация пакета "Данные на сервер (запись)"
    QByteArray createDataToServerPackage();
